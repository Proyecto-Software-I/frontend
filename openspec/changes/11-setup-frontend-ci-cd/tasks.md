...
- [ ] 1.1 Create `.github/workflows/ci.yml` for Pull Requests targeting `main`.
- [ ] 1.2 Configure the workflow to check out the repository.
- [ ] 1.3 Configure Node.js 24 with npm cache when appropriate.
- [ ] 1.4 Configure an explicit `npm ci` step.
- [ ] 1.5 Configure an explicit `npm run spec:validate` step.
- [ ] 1.6 Configure an explicit `npm run lint` step.
- [ ] 1.7 Configure an explicit `npm run build` step.
- [ ] 1.8 Confirm there is still no applicable automated test script at implementation time; do not add `npm test` unless an applicable suite appears within the approved scope.
- [ ] 1.9 Keep the workflow limited to validation and do not add deployment tokens, deployment commands, branch protection changes, ruleset changes, or personal-token workarounds.
...
- [ ] 3.1 Update `README.md` with a concise CI/CD section describing `PR -> main -> GitHub Actions -> npm ci -> OpenSpec validation -> lint -> build`.
- [ ] 3.2 Document `PR/branch -> Vercel Preview` and `main -> Vercel Production`.
- [ ] 3.3 Document the Vercel decision and summarize why Firebase App Hosting is technically valid but not selected under the issue constraints.
- [ ] 3.4 Document required external permissions for connecting Vercel to the GitHub repository and configuring Production Branch and Environment Variables.
- [ ] 3.5 Document that `NEXT_PUBLIC_API_URL` is public browser-exposed configuration, should be configured in Vercel for Preview and Production, and must not contain secrets.
- [ ] 3.6 Document that `.env.example` only changes if a new variable is introduced; do not change it just for Vercel setup.
- [ ] 3.7 Document backend CORS as a coordination risk if deployed Vercel URLs cannot reach the backend.
- [ ] 3.8 Document that GitHub rulesets, branch protections, backend CI/CD, custom domains, and paid services are outside this issue's scope.

## 4. Verification

- [ ] 4.1 Run `openspec validate 11-setup-frontend-ci-cd --strict --no-interactive` after planning changes and after implementation changes that affect OpenSpec.
- [ ] 4.2 Run `git diff --check`.
- [ ] 4.3 Run `npm run spec:validate`.
- [ ] 4.4 Run `npm run lint`.
- [ ] 4.5 Run `npm run build`.
- [ ] 4.6 Run `npm run check`.
...
- [ ] 4.16 Confirm no secrets, tokens, API keys, `.env` files, Vercel local state, Firebase configuration, or credentials are committed.
- [ ] 4.17 Confirm no frontend routes, components, responsive behavior, accessibility behavior, loading states, error states, backend files, or backend HTTP contracts changed.
