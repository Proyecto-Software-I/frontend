...
- [x] 1.1 Generate `src/components/ui/dropdown-menu.tsx` with shadcn `DropdownMenu` using the existing unified `radix-ui` dependency; add no dependency. Verify generated checks; manually test arrows, Escape, and focus restoration. Rollback: remove this primitive with Unit 1.

## Phase 2: Switcher Behavior

- [x] 2.1 Create `src/features/workspace/components/organization-switcher.tsx` using `useAuth`, canonical `Membership`, and `ACTIVE_MEMBERSHIP_STATUS`; retain static context below two active memberships. Verify active-only filtering; manually confirm no inactive option. Rollback: remove this component with Unit 1.
- [x] 2.2 Add different-selection delegation, a synchronous single-flight guard acquired before Auth invocation and released only on settlement, local open/pending/error state, active-item no-op, successful dropdown close before `router.replace("/dashboard")`, and retryable denial that keeps the relevant controls open without tenant state. Verify request/no-op, disabled pending, and preserved denied context; manually retry denial then confirm updated roles. Rollback: revert this behavior with the switcher.
...
- [x] 3.1 Modify `src/features/workspace/components/workspace-shell.tsx` for desktop-header and existing mobile-aside placement without navigation changes; after successful mobile selection, close its containing aside/overlay before redirect. Verify both use one Auth session; manually test both widths, focus, success close order, and denial in an open aside. Rollback: restore static shell context.
- [x] 3.2 Modify `tests/auth-provider.state.test.tsx` for deferred success and denied selection, proving atomic adoption and unchanged session, token, organization, and roles. Verify focused tests pass. Rollback: revert tests with the integration slice.

## Phase 4: Final Verification

- [x] 4.1 Run `npm run lint`, `npm run build`, and strict OpenSpec validation. Verify all pass; manually confirm desktop/mobile success, loading, denial, keyboard flow, and no console or hydration errors. Rollback: revert the failing work unit only.
